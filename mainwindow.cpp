    ...
    if (initFTP()) {
        ...
        if (!string_list.isEmpty()) {
            for (int i = 0; i < string_list.count(); i++) {
                ...
                QUrl url(m_ftpPath);
                url.setPort(21);
                QString name  =ui->lineEdit_user->text(),password=ui->lineEdit_password->text();
                if(name.isEmpty()==false){
                    url.setUserName(ui->lineEdit_user->text());
...
void MainWindow::on_Btn_download_clicked()
{
    if (initFTP()) {
        QString savePath;
        savePath = QFileDialog::getSaveFileName(this, tr("保存文件"));
